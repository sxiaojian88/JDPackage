...
class JDWrapper(object):
	...
	def buy(self,options):
		...
		self.cart_detail()
		return self.order_info(options)
	...
	def order_info(self, options):
		...
		try:
			...
			timetext = today + ' ' + options.time
			timeStart = datetime.datetime.strptime(timetext, '%Y-%m-%d %H:%M:%S')
			enabled = 0
			while enabled == 0:
				if datetime.datetime.now() >= timeStart:
